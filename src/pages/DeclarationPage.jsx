...
const DeclarationPage = () => {
  ...
  const [shareCounts, setShareCounts] = useState(() => {
    const saved = localStorage.getItem('declaration_share_counts');
    return saved ? JSON.parse(saved) : {};
  });
  
  const [downloadCounts, setDownloadCounts] = useState(() => {
    const saved = localStorage.getItem('declaration_download_counts');
    return saved ? JSON.parse(saved) : {};
  });
  
  const [viewCounts, setViewCounts] = useState(() => {
    const saved = localStorage.getItem('declaration_view_counts');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('declaration_share_counts', JSON.stringify(shareCounts));
  }, [shareCounts]);

  useEffect(() => {
    localStorage.setItem('declaration_download_counts', JSON.stringify(downloadCounts));
  }, [downloadCounts]);

  useEffect(() => {
    localStorage.setItem('declaration_view_counts', JSON.stringify(viewCounts));
  }, [viewCounts]);
          ...
          <div className="min-h-screen px-4 py-8">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <Megaphone size={14} className="text-[#4CAF50] mb-2" />
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button 
                      ...
                      className="px-4 py-2 bg-white border border-orange-500 text-orange-600 text-sm rounded-lg hover:bg-orange-500 hover:text-white flex items-center gap-2"
                    >
      ...
      {showShareModal && selectedDocument && (
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={`${window.location.origin}/legal/declaration/${selectedDocument.id}`} readOnly className="flex-1 px-3 py-2 border rounded-lg text-sm bg-gray-50" />

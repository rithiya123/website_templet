...
import {
  ...
  Megaphone, ChevronRight as ChevronRightIcon
} from "lucide-react";
...
const DeclarationPage = () => {
  ...
  const { loading, documents, totalPages, total } = useLegalDocuments(page, 10, "declaration");
  ...
  const handleCopyLink = () => {
    const url = `${window.location.origin}/legal/declaration/${selectedDocument?.id}`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                <div key={doc.id} className="bg-white rounded-xl border border-gray-200 hover:shadow-md hover:border-[#4CAF50]/30 transition-all duration-200 cursor-pointer overflow-hidden" onClick={() => handleDocumentClick(doc)}>
                  <div className="flex flex-col sm:flex-row p-4 gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center text-xs text-gray-400 mb-2"><Calendar size={12} className="mr-1" /><span>{formatDate(doc.publishedDate)}</span>{doc.documentNumber && <><span className="mx-2">•</span><Megaphone size={12} className="mr-1" /><span>{doc.documentNumber}</span></>}</div>
      ...
      {/* Modal and Share Modal similar to others with orange colors */}
